...
Geometry answers the third question. The threshold is a separate two-point line, and the app never derives it from the polygon: the same gate serves both directions, and a polygon has no side that a user can read as "in".
